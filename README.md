...
이 프로젝트는 최종 성능 수치만을 남기기보다, **분석 중심 접근에서 도메인 결정 구조 기반 모델링으로 관점이 바뀐 과정**을 함께 기록했습니다.
...
이번 프로젝트의 가장 큰 배움은 **데이터를 많이 결합하는 것과 타깃이 결정되는 구조를 모델링하는 것은 다르다**는 점이었습니다.

초기 접근은 다음에 가까웠습니다.

```text
데이터 결합 → 전처리 → CatBoost/XGBoost → LAST_GRADE 직접 예측
```

하지만 본선 진출자들의 접근을 비교하면서, 한우 등급 예측은 최종 등급을 바로 맞히는 단순 다중분류 문제가 아니라는 점을 확인했습니다. 실제로는 육질, 육량, 등외 여부, 혈통/유전 능력, 생애주기별 기상 스트레스가 결합되어 최종 등급이 결정됩니다.

따라서 더 발전된 접근은 다음과 같았어야 합니다.
...
즉, 우리는 데이터를 설명하려 했고, 상위 접근은 현상이 결정되는 구조를 모델로 재현하려 했습니다.

자세한 회고는 [`docs/retrospective.md`](docs/retrospective.md)에 정리했습니다.
...
또한 16개 클래스를 직접 예측하는 방식만으로는 A/B/C 세부 등급, 특히 C계열을 안정적으로 맞히기 어려웠습니다. 향후에는 품질축과 수율축을 분리한 multi-stage 모델, C-risk 모델, 체중-나이-성별 상대지표, 생애주기별 누적 기상 스트레스 파생변수를 추가로 설계할 필요가 있습니다.

> 처음에는 분석 중심으로 접근했지만, 결과를 비교하면서 예측 문제는 단순히 데이터를 모델에 넣는 것이 아니라 도메인 결정 구조를 모델링하는 관점이 필요하다는 것을 배웠습니다.
